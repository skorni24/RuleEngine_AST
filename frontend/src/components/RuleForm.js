...
const RuleForm = () => {
    ...
    const validateRuleString = (expression) => {
        ...
        const attributePattern = new RegExp(`\\b(${validAttributes.join('|')})\\b`, 'g');
        ...
        const attributeValuePairs = expression.match(/(\w+)\s*(==|!=|>|>=|<|<=)\s*('[^']*'|\d+|\d*\.\d+)/g);
    ...
    const convertLogicalExpression = (expression) => {
        ...
        // Replace single = with ===
        convertedExpression = convertedExpression.replace(/([^=!<>])=([^=])/g, '$1===$2');
    ...
    return (
        <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-2">Rule Name</label>
                    <input
                        id="name"
                        type="text"
                        value={name}
                ...
                <div>
                    <label htmlFor="expression" className="block text-sm font-medium text-gray-700 mb-2">Rule String</label>
                    <input
                        id="expression"
                        type="text"
                        value={expression}
                ...
                {error && <div className="text-red-600 bg-red-100 border border-red-300 rounded-md p-2 mt-2">{`Error: ${error}`}</div>}
                {success && <div className="text-green-600 bg-green-100 border border-green-300 rounded-md p-2 mt-2">{success}</div>}
            </form>
        </div>
    );
};
